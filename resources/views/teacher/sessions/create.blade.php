...
<div class="relative overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-br from-white via-white to-slate-50 p-6 mb-6">
 ...
 <h1 class="text-3xl sm:text-4xl font-extrabold text-slate-900 mt-3">إضافة جلسة</h1>
 <p class="text-sm text-slate-600 mt-2">تسجيل جلسة تسميع أو مراجعة لطالب</p>
 </div>
...
<x-card>
 <x-card-body>
 <form method="POST" action="{{ route('teacher.sessions.store') }}" class="space-y-6">
 @csrf
 ...
 <option value="{{ $student->id }}" data-track="{{ $student->track }}">{{ $student->name }}</option>
 @endforeach
 </x-select>

 {{-- Date --}}
 <x-input name="date" label="التاريخ" type="date" value="{{ old('date', now()->toDateString()) }}" required>
 <x-slot name="icon">
 <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 ...
 {{-- Session Type --}}
 <x-select name="session_type" label="نوع الجلسة" required>
 <x-slot name="icon">
 <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 ...
 سيتم ضبط نوع الجلسة تلقائياً بناءً على مسار الطالب (حفظ/تأسيس). يمكنك تغييره إذا لزم.
 </p>
 ...
 {{-- Ayah From --}}
 <x-input name="ayah_from" label="من آية" type="number" min="1" value="{{ old('ayah_from') }}">
 <x-slot name="icon">
 <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 ...
 {{-- Ayah To --}}
 <x-input name="ayah_to" label="إلى آية" type="number" min="1" value="{{ old('ayah_to') }}">
 <x-slot name="icon">
 <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4"/>
 </svg>
 ...
 @php
     $selectedSkillIds = collect(old('foundation_skill_ids', []));
     $selectedMastery = old('foundation_mastery', []);
 @endphp
 ...
 <input type="range" name="score" class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-primary-600" min="0" max="10" step="1" value="{{ old('score', 0) }}" oninput="document.getElementById('scoreValue').textContent = this.value">
 ...
 <span id="scoreValue" class="text-lg font-bold text-primary-600">{{ old('score', 0) }}</span>
 </div>
 </div>
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 if (!sessionTypeSelect || !memorizationFields || !foundationFields) {
 return;
 }

 function ensureHiddenSessionType(value) {
 let hidden = document.getElementById('sessionTypeHidden');
 if (!hidden) {
 hidden = document.createElement('input');
 hidden.type = 'hidden';
 hidden.name = 'session_type';
 hidden.id = 'sessionTypeHidden';
 sessionTypeSelect.parentNode.appendChild(hidden);
 }
 hidden.value = value || '';
 }

 function removeHiddenSessionType() {
 const hidden = document.getElementById('sessionTypeHidden');
 ...
 } else if (sessionType === 'new' || sessionType === 'revision') {
 ...
 const track = selectedOption ? selectedOption.getAttribute('data-track') : null;
 if (track === 'foundation') {
 sessionTypeSelect.value = 'foundation';
 sessionTypeSelect.setAttribute('disabled', 'disabled');
 ensureHiddenSessionType('foundation');
 } else if (track === 'memorization') {
 sessionTypeSelect.value = 'new';
 sessionTypeSelect.setAttribute('disabled', 'disabled');
 ensureHiddenSessionType('new');
 } else {
 sessionTypeSelect.removeAttribute('disabled');
 removeHiddenSessionType();
 }
 toggleFields();
 });
 }
